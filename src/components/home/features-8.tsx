...
import { Shield, Target, Lock, Activity, Users, Zap } from "lucide-react"
import { motion } from "framer-motion"

export function Features8() {
  return (
    <section id="features" className="bg-muted/30 py-16 md:py-32 dark:bg-transparent">
      <div className="mx-auto max-w-3xl lg:max-w-5xl px-6">
        <div className="relative">
          <div className="relative z-10 grid grid-cols-6 gap-3">
            <Card className="relative col-span-full flex overflow-hidden lg:col-span-2 group/card">
              <CardContent className="relative m-auto size-fit pt-6">
                <div className="relative flex h-24 w-56 items-center">
                  <svg className="text-muted-foreground/20 dark:text-muted-foreground/10 absolute inset-0 size-full transition-colors group-hover/card:text-success/10" viewBox="0 0 254 104" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
                ...
                <h2 className="mt-6 text-center text-2xl font-semibold">Self-Aware</h2>
                <p className="mt-2 text-center text-sm text-muted-foreground">Identifies hidden psychological patterns holding your trades back.</p>
              </CardContent>
            </Card>

            <Card className="relative col-span-full overflow-hidden sm:col-span-3 lg:col-span-2 group/card">
              <CardContent className="pt-6">
                <div className="relative mx-auto flex aspect-square size-32 rounded-full border border-border/50 before:absolute before:-inset-2 before:rounded-full before:border before:border-border/50">
                  <svg className="m-auto h-fit w-24" viewBox="0 0 212 143" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
                      className="text-muted-foreground/30 transition-colors group-hover/card:text-muted-foreground/60"
                      ...
                      fill="currentColor"
                    />
                    <path className="text-success/50 transition-colors group-hover/card:text-success" d="M3 72H209" stroke="currentColor" strokeWidth="6" strokeLinecap="round" />
                  </svg>
                </div>
                <div className="relative z-10 mt-6 space-y-2 text-center">
                  <h2 className="text-lg font-medium text-foreground transition group-hover/card:text-success">Capital Protection</h2>
                  <p className="text-sm text-muted-foreground">Hard stop-loss limits integrated directly through your broker's API.</p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="relative col-span-full overflow-hidden sm:col-span-3 lg:col-span-2 group/card">
              <CardContent className="pt-6">
                <div className="pt-6 lg:px-6">
                  <svg className="w-full text-muted-foreground/20 transition-colors group-hover/card:text-success/20" viewBox="0 0 386 123" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <rect width="386" height="123" rx="10" />
                    <circle className="text-muted/50" cx="29" cy="29" r="15" fill="currentColor" />
                    ...
                    <path
                        className="text-success transition-transform duration-500 group-hover/card:translate-x-2"
                ...
                <div className="relative z-10 mt-14 space-y-2 text-center">
                  <h2 className="text-lg font-medium transition text-foreground group-hover/card:text-success">Behavior Analytics</h2>
                  <p className="text-sm text-muted-foreground">Real-time alerts tracking revenge trading and emotional deviations.</p>
                </div>
              </CardContent>
            </Card>

            <Card className="relative col-span-full overflow-hidden lg:col-span-3 group/card">
              <CardContent className="grid pt-6 sm:grid-cols-2 h-full">
                <div className="relative z-10 flex flex-col justify-between space-y-12 lg:space-y-6">
                  <div className="relative flex aspect-square size-12 items-center justify-center rounded-xl bg-success/10 border border-success/20">
                    <Target className="size-5 text-success" strokeWidth={2} />
                  </div>
                  <div className="space-y-2 pb-6">
                    <h2 className="text-lg font-medium text-foreground transition group-hover/card:text-success">Position Sizing</h2>
                    <p className="text-sm text-muted-foreground">Automated calculations before you enter the trade. Never risk more than 1%.</p>
                  </div>
                </div>
                <div className="rounded-tl-2xl relative -mb-6 -mr-6 mt-6 h-fit border-l border-t border-border/50 bg-background/50 p-6 py-6 sm:ml-6 flex items-center justify-center min-h-[160px]">
                  <div className="flex flex-col gap-4 w-full">
                     <div className="flex justify-between items-center w-full px-2">
                         <span className="text-xs text-muted-foreground">Risk per trade</span>
                         <span className="text-xs font-bold text-success border border-success/30 bg-success/10 px-2 py-0.5 rounded text-center">₹500</span>
                     </div>
            ...
            <Card className="relative col-span-full overflow-hidden lg:col-span-3 group/card">
              <CardContent className="grid h-full pt-6 sm:grid-cols-2">
                <div className="relative z-10 flex flex-col justify-between space-y-12 lg:space-y-6">
                  <div className="relative flex aspect-square size-12 items-center justify-center rounded-xl bg-success/10 border border-success/20">
                    <Users className="size-5 text-success" strokeWidth={2} />
                  </div>
                  <div className="space-y-2 pb-6">
                    <h2 className="text-lg font-medium transition text-foreground group-hover/card:text-success">Accountability </h2>
                    <p className="text-sm text-muted-foreground">Join the 30-day challenge and trade alongside disciplined professionals.</p>
                  </div>
                </div>
                <div className="relative mt-6 sm:-my-6 sm:-mr-6 border-l border-border/50 bg-background/50 pl-6">
                  <div className="relative flex h-full flex-col justify-center space-y-6 py-6">
                    <div className="relative flex w-[calc(60%)] items-center justify-end gap-3 translate-x-4">
                      <span className="block h-fit rounded border border-border/50 bg-background px-3 py-1.5 text-xs shadow-sm font-medium">Profit Maker</span>
                      <div className="ring-background size-8 ring-4 bg-success/20 rounded-full flex items-center justify-center border border-success/30">
                         <Activity className="size-4 text-success" />
                      </div>
                    </div>
                    <div className="relative ml-0 flex items-center gap-3">
                      <div className="ring-background size-10 ring-4 bg-amber-500/20 rounded-full flex items-center justify-center border border-amber-500/30">
                        <Lock className="size-5 text-amber-500" />
                      </div>
                      <span className="block h-fit rounded border border-border/50 bg-background px-3 py-1.5 text-xs shadow-sm font-medium">Over-trading Box</span>
                    </div>
                    <div className="relative flex w-[calc(60%)] items-center justify-end gap-3 translate-x-4">
                      <span className="block h-fit rounded border border-border/50 bg-background px-3 py-1.5 text-xs shadow-sm font-medium">Risk Manager</span>
                      <div className="ring-background size-8 ring-4 bg-blue-500/20 rounded-full flex items-center justify-center border border-blue-500/30">
